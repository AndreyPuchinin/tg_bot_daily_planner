...
def info_handler(message):
    ...
    text += "⚠️ Для полноценной работы, чтобы получать напоминия, запустите @My_Diary_Reminder_Bot\n\n"
    ...
    text += "  – <i>за день в 6:00 по МСК,</i>\n"
    text += "  – <i>или за 12 часов до начала.</i>\n"
    text += "  – <i>можно изменить оба условия с помощью /settings</i>\n"
...
def handle_feedback_message(msg):
    ...
    for admin_id_str in ADMIN_USER_ID:
        try:
            ...
            bot.send_message(admin_id_str, message_to_admins, parse_mode="HTML")
        except ValueError:
